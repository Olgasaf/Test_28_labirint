...
def web_browser(request, selenium):
    ...
    # Сделайте снос (этот код будет выполняться после каждого теста):
...
def pytest_collection_finish(session):
    """ Эта функция изменяла имена тестовых примеров "на лету".
    ...
    if session.config.option.collectonly is True:
        for item in session.items:
            # Если в тестовом примере есть строка документа, нам нужно изменить его имя на
            # # # это строка документа, чтобы показывать удобочитаемые отчеты и
